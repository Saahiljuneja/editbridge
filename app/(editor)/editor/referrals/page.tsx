...
export default function EditorReferralsPage() {
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-5">
        <div className="px-8 py-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">Invite clients to EditBridge, earn ₹100 credits per referral</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-8 py-6 space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              { step: "1", title: "Share your link", desc: "Send your unique referral link to a creator or content team." },
              { step: "2", title: "They sign up", desc: "Your referral creates their free EditBridge account." },
              { step: "3", title: "You earn ₹100", desc: "When they place their first order worth ₹1000+, you get ₹100 credits." },
            ].map(({ step, title, desc }) => (
              <div key={step} className="text-center">
                <div className="w-9 h-9 rounded-full bg-[var(--brand-editor)] text-white text-sm font-bold flex items-center justify-center mx-auto mb-3">
                  {step}
                </div>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    copied
                      ...
                      : "bg-[var(--brand-editor)] text-white hover:bg-[var(--brand-editor-hover)]"
                  )}
                >
        ...
        {data && (
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              { label: "Credits Earned", value: formatCurrency(data.totalEarned), icon: IndianRupee, color: "text-[var(--brand-editor)]", bg: "bg-[var(--brand-editor)]/5" },
            ].map(({ label, value, icon: Icon, color, bg }) => (
              <div key={label} className={cn("rounded-2xl border border-gray-100 p-5 text-center shadow-sm bg-white", bg)}>
          ...
          <div className="flex items-center gap-3 rounded-xl bg-amber-50 border border-amber-100 px-5 py-4">
            ...
            <div>
              ...
              <p className="text-xs text-amber-600 mt-0.5">
                Waiting for them to place their first order. Credits will be awarded automatically.
              </p>
            </div>
          </div>
        )}
