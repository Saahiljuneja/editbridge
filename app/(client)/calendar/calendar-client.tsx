              ...
              return (
                <button key={i} onClick={() => setSelected(isSelected ? null : key)}
                  className={cn(
                    ...
                    isSelected ? "bg-[#0EA5E9]/10 ring-1 ring-[#0EA5E9]/30" : "hover:bg-gray-50",
                    events.length > 0 && !isSelected ? "bg-sky-50/50" : "",
                  )}>
                  <span className={cn(
                    "text-sm font-medium w-7 h-7 flex items-center justify-center rounded-full",
                    isToday ? "bg-[#0EA5E9] text-white" : isPast ? "text-gray-300" : "text-gray-700",
                  )}>
                    {day}
                  </span>
                  {events.length > 0 && (
          ...
          <div className="divide-y divide-gray-50">
            {selectedOrders.map((o) => (
              <Link key={o.id} href={`/orders/${o.id}`}
                ...
                <span className="text-xs font-semibold text-[#0EA5E9]">View →</span>
              </Link>
            ))}
          </div>
        </div>
