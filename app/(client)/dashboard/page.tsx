...
import {
  ...
  Search, AlertTriangle, CheckCircle, IndianRupee,
    ...
    <div className="min-h-screen bg-gray-50/60">
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-5">
        <div className="px-8 py-6 flex items-center justify-between gap-4">
          <div>
            <h1 className="text-xl font-bold text-gray-900">{greet}, {firstName} ðŸ‘‹</h1>
            <p className="text-sm text-gray-400 mt-0.5">
              {activeOrders.length > 0
                ? `You have ${activeOrders.length} active order${activeOrders.length !== 1 ? "s" : ""}`
                : "No active orders â€” ready to find an editor?"}
            </p>
          </div>
      ...
      <div className="px-8 py-6 space-y-6">
        ...
        <div className="grid lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
                ...
                <Link href="/orders" className="text-xs font-medium flex items-center gap-1 hover:underline" style={{ color: COLOR }}>
                      ...
                      <Link key={order.id} href={`/orders/${order.id}`}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            {s && <span className={cn("text-[10px] font-semibold px-1.5 py-0.5 rounded-full", s.badge)}>{s.label}</span>}
            ...
            {savedEditorsList.length > 0 && (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
                  ...
                  <Link href="/saved" className="text-xs font-medium flex items-center gap-1 hover:underline" style={{ color: COLOR }}>
                    View all <ArrowRight className="w-3 h-3" />
                  </Link>
                </div>
                <div className="divide-y divide-gray-50">
                  {savedEditorsList.map(editor => {
                    ...
                    return (
                      <div key={editor.editorId} className="flex items-center gap-3 px-5 py-3.5">
                        <div className="w-9 h-9 rounded-xl overflow-hidden bg-gradient-to-br from-sky-400 to-indigo-500 flex items-center justify-center shrink-0">
                          {editor.image ? (
                            // eslint-disable-next-line @next/next/no-img-element
                            <img src={editor.image} alt={shownName} className="w-full h-full object-cover" />
                          ) : (
                            <span className="text-white font-bold text-xs">{shownName.slice(0, 2).toUpperCase()}</span>
                          )}
                        </div>
                        <div className="flex-1 min-w-0">
                        ...
                        {lastOrder ? (
                          <Link
                            ...
                            className={cn(
                              "flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold text-white shrink-0 transition-opacity",
                              isAway || !editor.isAvailable ? "bg-gray-300 pointer-events-none" : "hover:opacity-85"
                            )}
                            style={isAway || !editor.isAvailable ? undefined : { background: COLOR }}
                          >
                          ...
                          <Link
                            ...
                            className="px-3 py-1.5 rounded-lg text-xs font-semibold text-white shrink-0 hover:opacity-85 transition-opacity"
                            style={{ background: COLOR }}
                          >
                            View
                          </Link>
            ...
            {recentReviews.length > 0 && (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
                  ...
                  <Link href="/reviews" className="text-xs font-medium hover:underline" style={{ color: COLOR }}>View all</Link>
                </div>
                <div className="divide-y divide-gray-50">
          ...
          {/* â”€â”€ Right col â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
          <div className="space-y-4">

            {/* Browse CTA if no orders */}
            {activeOrders.length === 0 && (
              <div className="rounded-2xl p-5 text-white" style={{ background: `linear-gradient(135deg, ${COLOR}, #6366f1)` }}>
                <Film className="w-6 h-6 mb-3 opacity-80" />
                <p className="font-semibold text-sm mb-1">Discover editors</p>
                <p className="text-xs opacity-75 mb-4 leading-relaxed">Browse portfolios, compare packages, and find the perfect editor for your project.</p>
                <Link href="/browse"
