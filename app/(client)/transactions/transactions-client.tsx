          ...
          {(["all", "completed", "active", "cancelled"] as const).map((f) => (
            <button key={f} onClick={() => setFilter(f)}
              className={cn("px-3 py-1.5 rounded-lg text-xs font-semibold capitalize transition-colors",
                filter === f ? "bg-[#0EA5E9] text-white" : "text-gray-500 hover:bg-gray-50")}>
              {f}
            </button>
          ))}
        </div>
        <div className="flex-1 flex items-center gap-2 sm:justify-end w-full sm:w-auto">
          <div className="relative flex-1 sm:flex-none sm:w-48">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)}
              placeholder="Search…"
              className="w-full rounded-lg border border-gray-200 pl-8 pr-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-[#0EA5E9]/20" />
          </div>
