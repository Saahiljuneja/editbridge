    ...
    <div className="px-8 py-6 space-y-5">
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search by client, order ID, package, or payment IDâ€¦"
          className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-200 bg-white"
        />
      </div>
      ...
      <div className="rounded-2xl border border-gray-100 bg-white shadow-sm overflow-hidden">
        <div className="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between">
          <p className="font-semibold text-gray-900 text-sm">Refundable orders</p>
          <span className="text-xs text-gray-400">{filtered.length} orders</span>
        </div>

        {loading ? (
          <div className="py-12 text-center text-sm text-gray-400 animate-pulse">Loadingâ€¦</div>
        ) : filtered.length === 0 ? (
              ...
              {filtered.map((order) => (
                <tr key={order.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/60 transition-colors">
                  <td className="px-5 py-3.5">
                    <p className="font-medium text-gray-900">{order.clientName ?? "Unknown"}</p>
                  ...
                  <td className="px-4 py-3.5 text-right">
                    <button
                      ...
                      className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold text-red-600 border border-red-200 bg-red-50 hover:bg-red-100 transition-colors"
                    >
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 w-full max-w-md p-6">
            <p className="font-bold text-gray-900 mb-1">Issue refund</p>
            <p className="text-xs text-gray-400 mb-5">
              {selected.packageTitle ?? "Custom order"} Â· {selected.clientName}
            </p>

            <form onSubmit={handleRefund} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">
                  Refund amount (â‚¹) â€” full order is {formatCurrency(selected.totalAmount)}
                </label>
                <input
                  type="number"
                  step="0.01"
                  ...
                  className="w-full rounded-xl border border-gray-200 px-3.5 py-2.5 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-red-200"
                  required
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  className="w-full rounded-xl border border-gray-200 px-3.5 py-2.5 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-red-200"
                  required
                  minLength={5}
                />
              </div>
              <div className="flex gap-3 pt-1">
                ...
                >
                  {submitting ? "Processingâ€¦" : "Confirm refund"}
                </button>
                <button
                  type="button"
