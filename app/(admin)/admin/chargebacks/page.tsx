    ...
    <div className="px-8 py-6 space-y-8">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Payment gateway disputes from Razorpay. Respond within 7 days to avoid automatic loss.</p>
        </div>
        <a href="https://dashboard.razorpay.com/app/disputes" target="_blank" rel="noopener noreferrer"
          className="flex items-center gap-1.5 text-xs font-semibold text-gray-500 border border-gray-200 px-3 py-2 rounded-xl hover:border-gray-300 transition-colors">
      ...
      <div className="rounded-2xl border border-gray-100 bg-white shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
          <p className="text-sm font-semibold text-gray-900">Chargeback Cases</p>
          <span className="text-xs text-gray-400 italic">Sample data â€” connect webhook to populate</span>
        </div>
        <div className="divide-y divide-gray-50">
          {MOCK_CHARGEBACKS.map(cb => (
            <div key={cb.id} className="flex items-center gap-4 px-6 py-4">
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-gray-900">{cb.paymentId}</p>
                <p className="text-xs text-gray-400">{cb.reason}</p>
              </div>
              <p className="text-sm font-bold text-gray-900 shrink-0">â‚¹{(cb.amount / 100).toFixed(2)}</p>
              <p className="text-xs text-gray-400 shrink-0">{cb.raisedAt}</p>
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm space-y-3">
        <p className="font-semibold text-gray-900 text-sm">How to respond to a chargeback</p>
