  ...
  return (
    <div className="px-8 py-6 ">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-0.5">Content oversight and review management.</p>
      </div>

      <div className="grid sm:grid-cols-2 gap-4 mb-8">
        <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
          <p className="text-sm text-gray-500 mb-1">Blocked messages</p>
      ...
      {/* Flagged portfolio comments */}
      <h2 className="font-semibold text-gray-900 mb-4">Flagged portfolio comments</h2>
      <div className="rounded-2xl border border-gray-100 overflow-hidden bg-white shadow-sm mb-10">
        {flaggedComments.length === 0 ? (
          <p className="px-5 py-10 text-center text-gray-400 text-sm">No flagged comments.</p>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="text-left px-4 py-3 font-medium text-gray-500">Portfolio item</th>
            ...
            <tbody>
              {flaggedComments.map(row => (
                <tr key={row.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition-colors">
                  <td className="px-5 py-3 font-medium text-gray-900">{row.authorName ?? "Unknown"}</td>
                  <td className="px-4 py-3 text-gray-500 text-xs max-w-[200px] truncate">{row.text}</td>
                  <td className="px-4 py-3 text-gray-400 text-xs">{row.itemTitle ?? "Untitled"}</td>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {recentReviews.map((row) => (
              <tr key={row.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition-colors">
                <td className="px-5 py-3 font-medium text-gray-900 truncate max-w-[120px]">{row.packageTitle}</td>
                <td className="px-4 py-3 text-gray-500">{row.reviewerName ?? "Unknown"}</td>
                ...
                <td className="px-4 py-3 text-gray-500 text-xs max-w-[180px] truncate">
                  {row.text ?? <span className="italic text-gray-300">No text</span>}
                </td>
                <td className="px-4 py-3 text-gray-400 text-xs">{formatDate(row.createdAt)}</td>
                <td className="px-4 py-3 text-right">
            ...
            {recentReviews.length === 0 && (
              <tr>
                <td colSpan={6} className="px-5 py-10 text-center text-gray-400 text-sm">No reviews yet.</td>
              </tr>
            )}
          </tbody>
        </table>
