...
import { X, Search, CheckCircle2, Sparkles } from "lucide-react";
import { cn } from "@/lib/utils";
import { toast } from "sonner";

const PRESET_SKILLS: Record<string, { emoji: string; items: string[] }> = {
  "Video Editing": {
    emoji: "🎬",
    items: [
      "YouTube Video Editing", "Reels Editing", "Shorts Editing",
      "Podcast Editing", "Wedding Video Editing", "Corporate Video Editing",
      "Documentary Editing", "Music Video Editing", "Ad / Commercial Editing",
      "Gaming Video Editing", "Tutorial Video Editing", "Vlog Editing",
    ],
  },
  "Thumbnail Design": {
    emoji: "🖼️",
    items: [
      "YouTube Thumbnail Design", "Clickbait Thumbnails",
      "Minimalist Thumbnails", "Photo Manipulation", "Text Effects",
    ],
  },
  "Motion Graphics": {
    emoji: "✨",
    items: [
      "Intro / Outro Animation", "Lower Thirds", "Kinetic Typography",
      "Logo Animation", "Transition Effects", "Visual Effects (VFX)",
    ],
  },
  "Audio": {
    emoji: "🎙️",
    items: [
      "Audio Mixing", "Noise Removal", "Voiceover Sync",
      "Sound Design", "Music Sync",
    ],
  },
  "Color & Grade": {
    emoji: "🎨",
    items: ["Color Grading", "Color Correction", "LUT Creation", "Cinematic Look"],
  },
  "Subtitles": {
    emoji: "💬",
    items: [
      "Subtitle Adding", "Caption Styling", "Multi-language Subtitles",
      "Auto-caption Correction",
    ],
  },
};

const ALL_SKILLS = Object.entries(PRESET_SKILLS).flatMap(([cat, { items }]) =>
...
export function SkillTags({ tags, onChange, placeholder = "e.g. Drone footage editing", maxTags = 20, className, validateTag }: SkillTagsProps) {
  const [open, setOpen] = useState(false);
  const [activeCategory, setActiveCategory] = useState(Object.keys(PRESET_SKILLS)[0]);
  ...
  function toggle(skill: string) {
    ...
    if (tags.includes(skill)) removeTag(skill); else addTag(skill, true);
  }
  ...
  const searchResults = search.trim()
    ? ALL_SKILLS.filter(({ item }) => item.toLowerCase().includes(search.toLowerCase()))
    : null;

  const categories = Object.keys(PRESET_SKILLS);
    ...
    <div className={cn("space-y-3", className)} ref={containerRef}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {open ? "Browsing skills — click a skill to add" : "Browse & add skills"}
        </div>
          ...
          {searchResults ? (
            ...
            <div className="p-3 max-h-56 overflow-y-auto">
              {searchResults.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-gray-300 mt-1">Add it as a custom skill below</p>
                </div>
              ) : (
            ...
            <div className="flex" style={{ height: 260 }}>
              ...
              <div className="w-36 shrink-0 border-r border-gray-100 overflow-y-auto bg-gray-50/40 py-2">
                {categories.map((cat) => {
                  const { emoji } = PRESET_SKILLS[cat];
                  const count = PRESET_SKILLS[cat].items.filter((s) => tags.includes(s)).length;
                  return (
                    <button
                      key={cat}
                      type="button"
              ...
              <div className="flex-1 overflow-y-auto p-4">
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">
                  {PRESET_SKILLS[activeCategory].emoji} {activeCategory}
                </p>
                <div className="flex flex-wrap gap-2">
                  {PRESET_SKILLS[activeCategory].items.map((skill) => {
