        ...
        <div className="max-w-7xl mx-auto w-full py-16 grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            ...
            <motion.div style={{ y: headY }}>
              <h1 className="font-black tracking-[-0.04em] leading-[0.92]" style={{ fontSize: "clamp(3rem, 7vw, 7rem)" }}>
                {["Find your", "editor for"].map((line, li) => (
                  <div key={line} className="overflow-hidden">
                    <motion.span className="block text-gray-900"
                      initial={{ y: "100%" }} animate={{ y: 0 }}
                      transition={{ delay: 0.28 + li * 0.15, duration: 0.9, ease: [0.22, 1, 0.36, 1] }}>
                      {line}
                    </motion.span>
                  </div>
                ))}
                <div className="overflow-hidden h-[1em] mt-2">
            ...
            <motion.p initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.62, duration: 0.7, ease: [0.22,1,0.36,1] }}
              className="text-gray-400 text-lg mt-8 mb-10 leading-relaxed font-medium">
              India&apos;s only marketplace where every editor submits government-issued ID before accepting orders — and every rupee is held in escrow until you approve.
            </motion.p>
            ...
            <motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.78, duration: 0.75, ease: [0.22,1,0.36,1] }}
              ...
              <form action="/browse" method="get"
                className="flex items-center gap-2 bg-white border border-gray-200 rounded-2xl p-1.5 shadow-[0_4px_24px_rgba(0,0,0,0.08)] max-w-lg">
                <Search className="w-4 h-4 text-gray-400 ml-2.5 shrink-0" />
                <input
                  name="q"
                  type="text"
                  placeholder="Find your editor for YouTube, Reels…"
              ...
              <div className="flex flex-wrap items-center gap-x-3 gap-y-2 mt-3">
                {["YouTube editor", "Reels editor", "Podcast editor", "Thumbnails"].map((chip) => (
                  <a key={chip} href={`/browse?q=${encodeURIComponent(chip)}`}
                    className="inline-flex items-center gap-1.5 text-xs text-gray-500 bg-gray-50 hover:bg-gray-100 border border-gray-100 hover:border-gray-200 rounded-full px-3 py-1.5 transition-colors">
                    <Search className="w-3 h-3 text-gray-400" />{chip}
                  </a>
                ))}
              </div>
